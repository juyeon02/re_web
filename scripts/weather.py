...
try:
                            ...
                            if df_temp is not None and not df_temp.empty:
                                ...
                                try:
                                    ...
                                    if 'SI' in df_long.columns:
                                        print(
                                            f"     -> [알림] {period['name']} 'Long' 포맷 데이터 파싱 성공 (데이터 {len(df_long)}개)")
    ...
    if all_dataframes:
        ...
        if 'SI' not in final_df.columns or '발전기명' not in final_df.columns:
            ...
            # -----------------------------------------------------------------
            # ✨ [요청사항] '오늘' 날짜만 필터링하는 코드 추가
            # -----------------------------------------------------------------

            # 1. KST (한국 시간) 기준 '오늘' 날짜(date 객체) 구하기
            today_kst = datetime.datetime.now(ZoneInfo("Asia/Seoul")).date()

            # 2. 'DATETIME' 컬럼의 날짜 부분(.dt.date)이 '오늘'과 같은 것만 필터링
            filtered_output_df = final_output_df[
                final_output_df['DATETIME'].dt.date == today_kst
            ].copy()

            # 3. 필터링된 'filtered_output_df'를 저장
            if not filtered_output_df.empty:
                filtered_output_df.to_csv(OUTPUT_FILE, index=False, encoding=OUTPUT_ENCODING)

                print(f"--- {today_kst} 날짜의 예보 {len(filtered_output_df)}건만 '{OUTPUT_FILE}' 파일로 저장했습니다. ---")

                print("\n데이터 미리보기")
                print(filtered_output_df.head())
            else:
                print(f"--- {today_kst} 날짜의 예보가 API에 아직 없습니다. (파일 업데이트 안함) ---")
                # (참고: 오늘 데이터가 없으면 'today_forecast...csv' 파일이 덮어쓰이지 않고,
                #        대시보드에는 이전에 저장된 (어제) 데이터가 계속 보일 수 있습니다.)

            # -----------------------------------------------------------------
            # ✨ (기존의 to_csv 로직은 위 'if' 블록 안으로 이동했음)
            # -----------------------------------------------------------------
