...
RESULT_BASE = os.path.join(BASE_DIR, "src/models/ensemble/results/voting")
MODEL_BASE = os.path.join(BASE_DIR, "outputs/models/ensemble/voting")
...
# -----------------------------------------------------------
# 📌 발전기별 Voting Ensemble 학습 함수
# -----------------------------------------------------------
def train_voting(df_sub, gen_name):
    ...
    save_model(
        voting,
        model_name=f"VotingEnsemble_{gen_name}",
    ...
    pred_df.to_csv(os.path.join(RESULT_DIR, "prediction_results.csv"), index=False, encoding="utf-8-sig")

    # ============================================================
    # 📌 [C] Feature Importance (RF 기반)
    # ============================================================
    ...
    fi_df.to_csv(os.path.join(RESULT_DIR, "feature_importance.csv"), index=False, encoding="utf-8-sig")
    ...
    plt.title(f"{gen_name} - Voting Ensemble 실제값 vs 예측값")
    plt.grid(True)
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    for gen_name, group in df.groupby('발전기명'):
        ...
        if len(group) < 20:
            print(f"⚠️ {gen_name}: 데이터 부족({len(group)}행), 학습 스킵")
            continue

        metrics = train_voting(group, gen_name)
    ...
    print("🎉 모든 발전기 Voting Ensemble 학습 완료!")
